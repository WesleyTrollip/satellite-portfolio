...
[ApiController]
[Route("api/lookups/correction-reasons")]
public sealed class CorrectionReasonsController(CorrectionReasonLookupService service) : ControllerBase
{
    [HttpGet]
    ...
    {
        var item = await service.GetByIdAsync(new CorrectionReasonLookupId(id), cancellationToken);
    ...
    {
        var updated = await service.UpdateAsync(new CorrectionReasonLookupId(id), new UpsertLookupRequest(request.Code, request.Name, request.IsActive), cancellationToken);
    ...
    public async Task<ActionResult> Delete(Guid id, CancellationToken cancellationToken)
    {
        try
        {
            var deleted = await service.DeleteAsync(new CorrectionReasonLookupId(id), cancellationToken);
